...
const applyNewNodeToPortConnection = (graph, connection) => {
  ...
  const { nodes } = result;
  const fromNode = find(nodes, { id: connection.from.data.id });
  const fromPortId = `${targetPortType}_${fromNode.ports.filter(p => p.type !== connection.to.data.type).length + 1}`;
  fromNode.ports = [
    ...fromNode.ports,
    {
      id: fromPortId,
...
const applyNewPortToNodeConnection = (graph, connection) => {
  ...
  const toPortId = `${targetPortType}_${toNode.ports.filter(p => p.type !== connection.from.data.type).length + 1}`;
  toNode.ports = [
    ...toNode.ports,
    {
      id: toPortId,
